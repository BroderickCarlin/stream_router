...
fn single_poll_no_routing() {
    ...
    assert_eq!(Pin::new(&mut router).poll_next(&mut cx), Poll::Ready(Some(1)));
    assert_eq!(Pin::new(&mut router).poll_next(&mut cx), Poll::Ready(Some(2)));
...
fn single_poll_routing() {
    ...
    assert_eq!(Pin::new(&mut router).poll_next(&mut cx), Poll::Ready(Some(2)));
    assert_eq!(Pin::new(&mut router).poll_next(&mut cx), Poll::Ready(Some(4)));
    assert_eq!(Pin::new(&mut router).poll_next(&mut cx), Poll::Ready(None));
}
